        ...
        <div class="row">
            <div class="col-xl-7 ">
                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('quotations.index') }}" class="btn btn-sm btn-primary"><i class="fas fa-chevron-left"></i> {{ __('Back') }}</a>
                                @if ($pdFInvoice)
                   ...
                   @include('quotations.partials.orderinfo')
                   @include('quotations.partials.actions.buttons',['order'=>$order])
                </div>
            </div>
            <div class="col-xl-5  mb-5 mb-xl-0">
                ...
                <div class="card card-profile shadow">
                    ...
                    <div class="card-body">
                        @include('quotations.partials.map',['order'=>$order])
                    </div>
                </div>
                @endif
                <br/>
                <div class="card card-profile shadow">
                    ...
                    @include('quotations.partials.orderstatus')
                    
                </div>
                @if(auth()->user()->hasRole('client'))
                @if($order->status->pluck('alias')->last() == "delivered")
                    <br/>
                    @include('quotations.partials.rating',['order'=>$order])
                @endif
                @endif
            </div>
        </div>
        @include('layouts.footers.auth')
        @include('quotations.partials.modals',['order'=>$order])
    </div>
@endsection
